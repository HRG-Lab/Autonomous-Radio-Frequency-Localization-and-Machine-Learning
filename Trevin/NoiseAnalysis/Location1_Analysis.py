...
importDataSet('Location1_Set1.csv', isFirst=True)
importDataSet('Location1_Set3_Sideways.csv')
importDataSet('Location1_Set4_OnEnd.csv')
importDataSet('Location1_Set5_Rotated.csv')
importDataSet('Location1_Set6_Streaming.csv')
importDataSet('Location1_Set7_OppositeSideOfHall.csv')
...
fig, ax = plt.subplots()
ind = np.arange(np.count_nonzero(plotTracker))     # the x locations for the groups
width = 0.05                    # the width of the bars

for i in range(NUM_FILES):
    plotList = [signalMatrix[i][j] for j in np.nonzero(plotTracker)[0]]
    # print(plotList)
    p = ax.bar(ind+i*width, plotList , width)

ax.set_title('Effect of Laptop Position on Signal Strength Observed')
ax.set_xticks(ind + width*(NUM_FILES-1) / 2)
ax.set_xticklabels([sourceList[j] for j in np.nonzero(plotTracker)[0]])
for tick in ax.xaxis.get_major_ticks():
    tick.label.set_fontsize(12)
plt.xticks(rotation=20)
matplotlib.rcParams.update({'font.size': 12})

lgd = ax.legend(('Normal','Sideways','On End','Rotated','Streaming','Opposite Side Of Hall'),loc="lower right")
lgd.get_frame().set_alpha(1)
plt.ylabel("Signal Strength (dBm)")
# ax.yaxis.set_units(inch)
ax.autoscale_view()
# plt.xlim(plt.xlim()[0],plt.xlim()[1]+4)
