#!/usr/bin/python3
# Filename: RoverMvmt.py

# Reads data from two HC-SR04 Ultrasonic Sensors to determine
# the location of an object between them. The program will send
# movement commands to a Pixhawk controller that is serially
# connected to a raspberry pi via MavLink protocol based on the
# position of the object between the two sensors.

import time
import multiprocessing as mp
import RPi.GPIO as GPIO
import time
from dronekit import connect, VehicleMode, LocationGlobalRelative, LocationLocal, Command, LocationGlobal
from pymavlink import mavutil
#import mavproxy 
  

            
 #-- Define the function for sending mavlink velocity command in body frame
def set_velocity_body(vehicle, vx, vy, vz):
    """ Remember: vz is positive downward!!!
    http://ardupilot.org/dev/docs/copter-commands-in-guided-mode.html        
    Bitmask to indicate which dimensions should be ignored by the vehicle 
    (a value of 0b0000000000000000 or 0b0000001000000000 indicates that 
    none of the setpoint dimensions should be ignored). Mapping: 
    bit 1: x,  bit 2: y,  bit 3: z, 
    bit 4: vx, bit 5: vy, bit 6: vz, 
    bit 7: ax, bit 8: ay, bit 9:
        
        
        """
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
            0,
            0, 0,
            mavutil.mavlink.MAV_FRAME_BODY_NED,
            0b00001111111000111, #-- BITMASK -> Consider only the velocities
            0, 0, 0,        #-- POSITION
            vx, vy, vz,     #-- VELOCITY
            0, 0, 0,        #-- ACCELERATIONS
            0, 0)
    vehicle.send_mavlink(msg)
    vehicle.flush()

def arm_and_takeoff(altitude):

   while not vehicle.is_armable:
      print"waiting to be armable"
      time.sleep(1)

   print"Arming motors"
   vehicle.mode = VehicleMode("GUIDED")
   vehicle.armed = True

   while not vehicle.armed: time.sleep(1)

   print"Taking Off"
   vehicle.simple_takeoff(altitude)

   while True:
      v_alt = vehicle.location.global_relative_frame.alt
      print(">> Altitude = %.1f m"%v_alt)
      if v_alt >= altitude - 1.0:
          print("Target altitude reached")
          break
      time.sleep(1)

        
if __name__ == "__main__":
    GPIO.setwarnings(False)
    #-- Connect to the vehicle
    print('Connecting...')
    vehicle = connect('udp:0.0.0.0:14550')
    #vehicle = connect('/dev/ttyUSB0', baud=57600)
    arm_and_takeoff(-1)
    #-- Setup the commanded speed
    gnd_speed = 5 # [m/s]
    DataBase=1
	
    while True:
    	if DataBase == 2:             
            print"Rover moves backward \n"  
            set_velocity_body(vehicle, -100, 0, 0)  
	elif DataBase == 1:   
            print"Rover moves foreward \n"
            set_velocity_body(vehicle, 100, 0, 0)
	else:
            print"Rover Halts \n"
            set_velocity_body(vehicle, 0, 0, 0)		
       
       
    print("Done!")
    print("Exiting Program")
    vehicle.close()

